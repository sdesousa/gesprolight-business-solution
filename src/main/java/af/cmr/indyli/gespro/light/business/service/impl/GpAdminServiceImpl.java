...
public class GpAdminServiceImpl implements IGpAdminService<GpAdmin> {
	...
	public void update(GpAdmin emp) throws GesproBusinessException {
		...
		GpAdmin existEmpl = this.empDAO.findById(emp.getId());
		if (existEmpl.getId() == null) {
			throw new GesproBusinessException(String.format("le Admin que vous voulez mettre en jour n'existe pas "));
		}
	...
	public void deleteById(Integer empId) throws GesproBusinessException {
		GpAdmin existEmpl = this.empDAO.findById(empId);
		if (existEmpl == null) {
			throw new GesproBusinessException(String.format("le Admin que vous voulez supprimer n'existe pas "));
		}
